    ...
    try:
        ...
        if num_accounts < 0:
            ...
            for i in range(num_accounts):
                ...
                acc_dict["Initial Balance"] = float(
                ...
                acc_dict["Contribution Frequency"] = list(contr_options.keys())[
                    terminal_menu.show()
                ]
                acc_dict["n"] = contr_options[acc_dict["Contribution Frequency"]]

                acc_dict["Contribution Amount"] = float(
                    input(
...
def generate_predicted_roi(accounts, after_years, growth_rates):
    predictions = {}
        ...
        for y in after_years:
            ...
            for ac in accounts:
                roi += return_on_investment(
                    ac["Initial Balance"], ac["n"], ac["Contribution Amount"], r, y
                )
...
def return_on_investment(B, n, M, R, t):
    """
    [return_on_investment] is the balance of your account after [t] years based on the other arguments
    B = initial amount
    n = number of contributions per year
    M = amount per contribution
    R = growth rate
    t = number of years
    """
    BeRt = B * math.exp(R * t)
